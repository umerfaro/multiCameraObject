...
def apply_homography_xyxy(xyxy, H):
    """
    Apply homography transformation to bounding boxes.

    Args:
        xyxy (np.ndarray): Array of bounding boxes in [x1, y1, x2, y2] format
        H (np.ndarray): Homography transformation matrix

    Returns:
        np.ndarray: Transformed bounding boxes
    """
    # Create an array to store transformed bounding boxes
    xyxy_ = np.zeros_like(xyxy)

    for idx, (x1, y1, x2, y2) in enumerate(xyxy[:, :4]):
        # Transform top-left corner
        p0 = H @ np.array([x1, y1, 1]).reshape(3, 1)
        x1_new, y1_new, s1 = p0.reshape(-1)
        x1_new /= s1
        y1_new /= s1

        # Transform bottom-right corner
        p1 = H @ np.array([x2, y2, 1]).reshape(3, 1)
        x2_new, y2_new, s2 = p1.reshape(-1)
        x2_new /= s2
        y2_new /= s2

        # Store the transformed coordinates
        xyxy_[idx, :4] = [x1_new, y1_new, x2_new, y2_new]
        
        # Preserve additional columns if present
        if xyxy.shape[1] > 4:
            xyxy_[idx, 4:] = xyxy[idx, 4:]
